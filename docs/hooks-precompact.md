...
ユーザーは何もする必要がありません。圧縮が起きたこと自体に気づかない場合もあります。`systemMessage` はエージェント（AI側）が読む情報であり、ユーザーに対して表示されるものではありません。フックを設定しておけば、圧縮時に自動で状態が引き継がれるため、ユーザーが改めて説明し直す必要がなくなります。
