# SKILL.md フロントマタープロパティ リファレンス

> 情報源: [VS Code 公式ドキュメント](https://code.visualstudio.com/docs/copilot/customization/agent-skills) + 拡張機能ソースコード
...
| `description` | string | **必須** | - | スキルの用途と使用タイミング。最大1024文字 |
...
| 設定 | `/` コマンド | モデル自動ロード | ユースケース |
|---|---|---|---|
| 両方省略（デフォルト） | 表示 | 有効 | 汎用スキル |
| `user-invocable: false` | 非表示 | 有効 | バックグラウンド知識スキル |
| `disable-model-invocation: true` | 表示 | 無効 | オンデマンド専用スキル |
